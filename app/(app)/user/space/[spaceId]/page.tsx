...
export default function SpaceInfo({ params } : { params: Promise<{ spaceId: string }>}) {
  ...
  const handleLike = async (testimonialId: string, state: boolean) => {
      try {
          const response = await axios.post(`/api/like/${testimonialId}`, {
              isLiked: !state
          });
          setTestimonials(prevTestimonials => 
              prevTestimonials.map(testimonial => 
                  testimonial.id === testimonialId 
                      ? { ...testimonial, isLiked: !state } 
                      : testimonial
              )
          );
          ...
          toast.error("Error", {
              description: "Error while liking the testimonial"
          })
      }
  }
